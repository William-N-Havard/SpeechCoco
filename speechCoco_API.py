...
class Timecode:
                    ...
                    for phonemeTimecode in phonemeAlignment :
                        ...
                        if phonemeBegin >= syllableBegin and phonemeEnd <= syllableEnd :
                            if wordTimecode['value']=='__SIL__':
                                updatedPhonemeTimecode =  {'value':'__'+phonemeTimecode["value"]+'__','begin':phonemeTimecode["begin"], 'end':phonemeTimecode["end"]}
                            else:
    ...
    def s_toTextgrid(timecodes, outputDir, wavName, level=3) :
        ...
        with open(outputDir, 'w') as f :
            ...
            if level >= 3 :
                # get phonemes
                phonemes = [phoneme for words in timecodes for syllables in words["syllable"] for phoneme in syllables["phoneme"]]
                nbTier = len(phonemes)
    ...
    def s_getWords(parsedTimecode, begin, end, level=1, olapthr=75):
        ...
        for word in parsedTimecode:
            ...
            if begin<=word['end'] and end>=word['begin']:
                ...
                if olap>=olapthr:
                    token.append({'word':word['value'].lower(), 'overlapPercentage':olap, 'begin':word['begin'], 'end':word['end']})
                    # syllable level
                    if level>=2:
                        for syllable in word['syllable']:
                            if begin<=syllable['end'] and end>=syllable['begin']:
                                if 'syllables' not in token[-1].keys():
                                    token[-1].update({'syllables':[{'value':syllable['value'], 'overlapPercentage':Caption._overlap(begin, end, syllable['begin'], syllable['end']),'begin':syllable['begin'], 'end':syllable['end']}]})
                                else:
                                    token[-1]['syllables'].append({'value':syllable['value'], 'overlapPercentage':Caption._overlap(begin, end, syllable['begin'], syllable['end']),'begin':syllable['begin'], 'end':syllable['end']})
                                # phoneme level
                                if level>=3:
                                    for phoneme in syllable['phoneme']:
                                        if begin<=phoneme['end'] and end>=phoneme['begin']:
                                            if 'phonemes' not in token[-1]['syllables'][-1].keys():
                                                token[-1]['syllables'][-1].update({'phonemes':[{'value':phoneme['value'], 'overlapPercentage':Caption._overlap(begin, end, phoneme['begin'], phoneme['end']),'begin':phoneme['begin'], 'end':phoneme['end']}]})
                                            else:
                                                token[-1]['syllables'][-1]['phonemes'].append({'value':phoneme['value'], 'overlapPercentage':Caption._overlap(begin, end, phoneme['begin'], phoneme['end']),'begin':phoneme['begin'], 'end':phoneme['end']})
        return token
...
class SpeechCoco:
    ...
    def filterCaptions(self, speaker=[], gender=[], disfluencyPos=[], nationality=[], speed=[], text=[], duration=lambda d:d>=0, raw=False):
        """
        :param speaker:
        :param gender:
        :param disfluencyPos:
        ...
        if len(speaker)!=0 or len(gender)!=0 or len(disfluencyPos)!=0 or len(nationality)!=0 or len(speed)!=0:
            whereQuery=SpeechCoco._buildQuery(speaker=speaker, gender=gender, disfluencyPos=disfluencyPos, nationality=nationality, speed=speed, text=text)
    ...
    def jsonToSQL(dirJsons, mergedFilename='./speechCoco.sqlite3', verbose=False):
        ...
        # Caption table
        db.execute('CREATE TABLE IF NOT EXISTS captions (captionID INTEGER PRIMARY KEY, imageID INTEGER, wavFilename TEXT, duration FLOAT, timecode TEXT, disfluencyPos TEXT, disfluencyVal TEXT, speed FLOAT, text TEXT, speaker TEXT)')
        ...
        for number,files in enumerate(filesInDir):
            ...
            with open(os.path.join(dirJsons,files)) as jsonFile:
                ...
                db.execute(imageInsert, (jsonData['captionID'],jsonData['imgID'],jsonData['wavFilename'],jsonData['duration'],json.dumps(jsonData['timecode']),jsonData['disfluency'][0],jsonData['disfluency'][1],jsonData['speed'],jsonData['synthesisedCaption'],jsonData['speaker']))
